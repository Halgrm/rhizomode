...
namespace Rhizomode.Core
{
    ...
    public abstract class NodeBase : IDisposable
    {
        ...
        {
            var definitions = new List<PortDefinition>();
            foreach (var kvp in _inputPorts)
                definitions.Add(new PortDefinition(kvp.Key, kvp.Value.Type, PortDirection.Input));
            foreach (var kvp in _outputPorts)
                definitions.Add(new PortDefinition(kvp.Key, kvp.Value.Type, PortDirection.Output));
            return definitions;
        }
