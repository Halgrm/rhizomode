...
namespace Rhizomode.Core
{
    ...
    public class GraphContext : IDisposable
    {
        ...
        public NodeBase? CreateNode(string nodeType)
        {
            ...
            {
                Debug.LogWarning($"[GraphContext] No factory for node type: {nodeType}");
                return null;
            }
        ...
        public bool TryConnect(string fromNodeId, string fromPort, string toNodeId, string toPort)
        {
            ...
            {
                Debug.LogWarning($"[GraphContext] Self-connection not allowed: {fromNodeId}");
                return false;
            }
            ...
            for (var i = 0; i < _edges.Count; i++)
            {
                ...
                {
                    Debug.LogWarning($"[GraphContext] Duplicate edge: {fromNodeId}.{fromPort} → {toNodeId}.{toPort}");
                    return false;
                }
            }
        ...
        public void Deserialize(GraphData data)
        {
            ...
            {
                Debug.LogWarning($"[GraphContext] Unknown graph version: {data.version} (expected 1.0)");
            }
            ...
            {
                Debug.LogError($"[GraphContext] Deserialize failed, restoring backup — {e.Message}");
        ...
        private void RestoreFromBackup(GraphData backup)
        {
            ...
            {
                Debug.LogError($"[GraphContext] Backup restore also failed — {e.Message}");
            }
        }
        ...
        public List<string> MergePreset(GraphData presetGraph, Vector3 spawnOffset)
        {
            ...
            foreach (var nodeData in presetGraph.nodes)
            {
                ...
                if (!_nodeFactories.TryGetValue(nodeData.type, out var factory))
                {
                    Debug.LogWarning($"[GraphContext] MergePreset: Unknown node type '{nodeData.type}'");
                    continue;
                }
                ...
                {
                    Debug.LogError($"[GraphContext] MergePreset node failed: {nodeData.type} — {e.Message}");
                }
            }
            ...
            foreach (var edgeData in presetGraph.edges)
            {
                ...
                {
                    Debug.LogWarning($"[GraphContext] MergePreset: Edge references unknown node");
                    continue;
                }
                ...
                {
                    Debug.LogWarning(
                        $"[GraphContext] MergePreset edge failed: {edgeData.fromPort} → {edgeData.toPort}");
                }
            }
