...
MaterialPropertyBlock使用。マテリアルインスタンス生成なし、メモリリークの心配なし。SRP Batcherは効かなくなるが、パフォーマンス規模では問題なし。
...
- 呼び出し: 左手Xボタン
- 表示方式: ボタン押下時の頭の正面位置にスポーン → ワールド空間に固定
- 操作完了後に自動で消える
...
- **方式**: 出力ポートからドラッグ → 入力ポートにスナップ
- **ポート直径**: 3cm
- **スナップ半径**: 10cm
- **スナップ優先**: 全ポートに対して最近接でスナップ。型が合うポートのみハイライト。型不一致はTryConnect時にバリデーションで弾く。
...
セカンドカメラの追加レンダリングは行わない。VRのレンダリング結果をBlit＋リサイズのみ。

### 8.2 Spout / NDI（後日）

RenderTextureの出力先としてSpout/NDI送信を追加。インターフェースだけ設計しておき、KlakSpout / KlakNDI 導入時に接続。
