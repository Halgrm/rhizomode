...
{
    /// <summary>
    /// CustomRenderTexture + CRT シェーダーでスペクトルを GPU 描画する。
    ...
    public class AudioSpectrumDisplay : MonoBehaviour
    {
        ...
        [SerializeField] private Color barColor = new(0.2f, 0.6f, 1f, 1f);
        [SerializeField, Range(0f, 4f)] private float intensity = 1.5f;
        [SerializeField, Range(0.001f, 0.05f)] private float barWidth = 0.01f;
        ...
        private void SetupCrt()
        {
            var shader = crtShader != null
                ? crtShader
                : Shader.Find("Rhizomode/CrtSpectrum");

            if (shader == null)
            {
                Debug.LogError("[AudioSpectrumDisplay] CrtSpectrum shader not found");
                return;
            }

            _crtMaterial = new Material(shader);
            _crtMaterial.SetColor("_BarColor", barColor);
            _crtMaterial.SetFloat("_Intensity", intensity);
            _crtMaterial.SetFloat("_Delta", barWidth);
